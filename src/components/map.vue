  ...
  <div class="map-content">
    <div class="map-service">
      <div class="map-service-right">
        ...
        <div class="text-fade">
          <p class="anim_fade" :style="{ animationDelay: (i+1)*3 + 's' }" v-for="(item,i) in offers" :key="i">{{item.successPosition}} </p>
          <p class="anim_fade" style="cursor:pointer" @click="routerLink('SuccessOffer',5)">更多…</p>
        </div>
      </div>
    </div>
  ...
  methods:{
    ...
    getSuccessfulCase(){
      ...
      this.$axios.post('/article/user/getSuccessfulCase').then(res=>{
        that.offers = res.data.data.slice(0,10);
      });
    },
  }
}
...
<style scoped lang="less" rel="stylesheet/less">
  ...
  *html .clearfix{height:1%}
  ...
  .map-service{position:relative;height:760px;background:url(../../static/images/map_balck_whole_bg.jpg) center no-repeat}
  .map-service-right{padding-top:60px}
  .china-map{float:left;width:748px;height:618px;background:url(../../static/images/map_black_bg.png) center no-repeat;position:relative;}
  ...
  .postition-9{left:637px;top:421px}
